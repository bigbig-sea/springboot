...
public class loginController {
    ...
    public String login(login login){
        String num=login.getNum();
        if (login.getNum()==null || login.getPassword()==null || login.getCode()==null){
            return "index";
        }
        //此处打印错误  返回正确或错误编码
        System.out.println(IdentityCardVerification(num));
        //此处为判断三者是否满足条件  进行页面跳转   密码和验证码由于时间暂时为固定值
        if (IdentityCardVerification(num)=="0" ||login.getPassword()=="123456"||login.getCode()=="123"){
            return "error";
        }
        return "index";
    }
    public static void main(String[] args) {
        ...
        System.out.println(IdentityCardVerification(login.getNum()));
    }
    /**
     *身份证验证
     * @param idStr
